...
public class Exercise2 {
    ...
    public void employersStuffList() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                                                   .flatMap(PersonJobPair::employeeToPersonJobPairStream)
                                                   .collect(groupingBy(p -> p.getPosition().getEmployer(),
                                                           mapping(p -> p.getPerson(), toSet())));
    ...
    public void indexByFirstEmployer() {
        ...
        Map<String, Set<Person>> result = employees.stream()
                                                   .map(employee -> new PersonJobPair(employee.getPerson(),
                                                           employee.getJobHistory().get(0)))
                                                   .collect(groupingBy(p -> p.getPosition().getEmployer(),
                                                           mapping(PersonJobPair::getPerson, toSet())));
    ...
    public void greatestExperiencePerEmployer() {
        ...
        Map<String, Person> collect = employees.stream().flatMap(employee -> employee.getJobHistory().stream()
                                                                                     .collect(toMap(e -> e.getEmployer(),
                                                                                             e -> new PersonDurationPair(
                                                                                                     employee.getPerson(), e.getDuration()),
                                                                                             (p1, p2) -> new PersonDurationPair(
                                                                                                     p1.getPerson(),
                                                                                                     p1.getDuration() + p2.getDuration())))
                                                                                     .entrySet()
                                                                                     .stream())
                                               .collect(toMap(Map.Entry::getKey, Map.Entry::getValue,
                                                       (e1, e2) -> e1.getDuration() > e2.getDuration() ? e1 : e2))
                                               .entrySet()
                                               .stream().
...
class PersonJobPair {
    ...
    private JobHistoryEntry position;

    static Stream<PersonJobPair> employeeToPersonJobPairStream(Employee employee) {
        return employee.getJobHistory().stream().map(jobHistoryEntry -> new PersonJobPair(employee.getPerson(), jobHistoryEntry));
    }
}
